...
export const MapGraphic = ({src, materialId, alt = "Geographic map", route = [], annotations = [], style}) => {
  if (!src) throw new Error("MapGraphic requires real map material or a cached map plate");
  if (!materialId) throw new Error("MapGraphic requires approved map materialId");
  return (
    <AbsoluteFill data-videoagent-map="true" data-videoagent-material-id={materialId} style={{overflow: "hidden", ...style}}>
