...
def shape_scenes(spans, target):
    """Seed editorial units from real narration segment boundaries.

    Short neighbouring speech segments may be merged to avoid fragmentation.
    Long segments are deliberately not divided into equal clock fractions: PLAN
    must place any further boundary at a meaningful editorial/visual beat.
    """
    merged = []
...
def step_plan(n, words_arg=None, fps=30, target=4.0, force=False):
    ...
    scenes = [{"id": f"S{index}", "startSec": round(start, 3), "endSec": round(end, 3),
               ...
               "visualTreatment": "", "materials": [],
               "status": "planned"}
              for index, (start, end) in enumerate(
                  shape_scenes(segments_from_words(words), target), start=1)]
    plan = {"schemaVersion": "v18-rebuilt-plan-v1", "video": f"V{n}", "fps": int(fps),
