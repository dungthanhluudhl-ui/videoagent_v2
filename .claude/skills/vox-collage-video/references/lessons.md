...
- Treating complex Pexels photos as automatic rembg inputs led to fuzzy, glitchy cutouts. Pexels photos are now actual visual plates by default; crop, reframe, layer, pan, push, or zoom them in Remotion. Use cutout processing only when a specific scene genuinely requires it, and use controlled chroma reconstruction only when truthful source material cannot communicate the need.
